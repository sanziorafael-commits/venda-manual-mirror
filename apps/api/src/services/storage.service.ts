...
export async function createStorageSignedUploadUrl(
  ...
  if (env.STORAGE_PROVIDER !== 'gcs') {
    throw badRequest('Storage de arquivos estÃ¡ desabilitado no ambiente atual');
  }
...
function parseCredentialsFromBase64(rawCredentialsBase64?: string) {
  ...
  try {
    ...
    logger.error({ err: error }, 'nÃ£o foi possÃ­vel decodificar GCS_CREDENTIALS_BASE64');
    throw badRequest('Credenciais GCS invÃ¡lidas');
  }
}
...
function parseCredentialsFromJson(rawJson?: string, sourceLabel?: string) {
  ...
  try {
    ...
    logger.error({ err: error, sourceLabel }, 'credenciais GCS invÃ¡lidas');
    throw badRequest('Credenciais GCS invÃ¡lidas');
  }
}
...
export function readServiceAccountFromFile(filePath: string) {
  try {
    ...
    logger.error({ err: error, filePath }, 'nÃ£o foi possÃ­vel ler o arquivo de credenciais GCS');
    throw badRequest('Arquivo de credenciais GCS invÃ¡lido');
  }
}
